...
output/02-steps.html: 02-steps.md assets/template/theme.css
	docker run --rm --init -v "$(PWD)":/home/marp/app/ -e LANG=${LANG} -e MARP_USER="${UID}:${GID}" marpteam/marp-cli 02-steps.md --theme-set assets/template/theme.css --html --allow-local-files -o output/02-steps.html
