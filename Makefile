...
output/%.html: slides/%.md assets/template/theme.css
	docker run --rm --init -v "$(PWD)":/home/marp/app/ -e LANG=${LANG} -e MARP_USER="${UID}:${GID}" marpteam/marp-cli:v4.1.2 $< --theme-set assets/template/theme.css --html --allow-local-files --pdf-option="--no-sandbox" --pdf-option="--disable-dev-shm-usage" -o $@
